...
- [x] Manual subscriptions (pay-the-owner-directly model) — `auth.grant_subscription()`, owner-only sidebar panel gated by `OWNER_EMAIL` secret. Bounded to a per-plan allowance (`auth.SUBSCRIPTION_SEARCH_LIMITS`: normal=100, pro=70, max=50 -- pricier tiers get a smaller cap since Opus costs more per search than Sonnet) so one subscriber can't exhaust the site-wide cap alone. Owner's own account is genuinely unlimited (`is_owner()`).
